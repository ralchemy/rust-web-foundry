...
Domain workflow outputs and tracked Domain documents own business understanding: language, capabilities, semantic ownership, boundaries, invariants, behaviors, and business type candidates. Root and nearest-local `AGENTS.md` files own standing governance, scope responsibility, and retained hard protection. A Guide chapter selected by `docs/agents/context-routes.tsv` and included in the compiled Context Pack owns its conditional engineering contract. Source, manifests, tests, Just recipes, and CI own executable facts. A generated Context Pack is only a read view and cannot override or duplicate these owners. A Domain output cannot override these engineering owners or mandate crate layout, dependencies, Ports, adapters, persistence, transport, resilience, or runtime design.
