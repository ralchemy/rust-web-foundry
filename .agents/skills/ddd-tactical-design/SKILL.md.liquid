...
1. Compile or reuse the bounded Context Pack for planned artifact or code paths with `domain-workflow` and `domain-modeling`, plus only other action keys that actually apply. Read the pack once.
2. Read the confirmed glossary, context map, capability documents, discovery or strategic output, and existing model selected by that context. Do not replace established language without evidence.
...
10. Define acceptance scenarios that prove the invariants. Before persisting confirmed durable semantics, add `domain-authority` and, when needed, `domain-capability-doc`, rebuild the pack, and read only new owner sections.
