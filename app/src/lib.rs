...
pub struct BuiltService { pub(crate) router: axum::Router, pub(crate) pool: MySqlPool }
impl BuiltService {
    pub fn router(&self) -> axum::Router { self.router.clone() }
    pub async fn close(self) { self.pool.close().await; fastrace::flush(); }
}
...
pub async fn build(config: BuildConfig) -> AppResult<BuiltService> {
    ...
    let router = {
        ...
        http::router(CreateTask::new(policy, repository.clone()), GetTask::new(repository.clone()), StartTask::new(repository), readiness, config.tracing_enabled)
    };
...
pub async fn run() -> AppResult<()> {
    ...
    match command { command::Command::Migrate => migrate().await, command::Command::Serve => serve().await }
}
...
async fn serve() -> AppResult<()> {
    ...
    observability::init_logging(settings.log_format, &settings.rust_log)?;
    let tracing_enabled = observability::init_tracing(settings.trace_exporter, settings.otlp_endpoint.as_deref(), settings.otlp_timeout, &settings.deployment_environment)?;
    let service = build(BuildConfig { database_url: settings.database_url,
        #[cfg(feature = "reference-task")] task_policy_url: settings.task_policy_url,
        #[cfg(feature = "reference-task")] task_policy_timeout: settings.task_policy_timeout,
        tracing_enabled }).await?;
    server::serve(service, settings.http_addr, settings.shutdown_timeout).await
}

pub(crate) fn fail(message: &'static str) -> Box<dyn Error + Send + Sync> { Box::new(io::Error::other(message)) }
