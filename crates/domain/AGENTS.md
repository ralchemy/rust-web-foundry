...
Before adding or changing parsing, formatting, conversion traits, ownership, or naming conventions, read `docs/guide/reference/idiomatic-rust.md`.
...
- Let each value type own parsing, validation, and formatting. Implement `FromStr` for a stable textual representation and `Display` for its canonical outward form; adapters call those traits instead of duplicating parsing or string mapping.
- Use named Domain operations such as `create` and `reconstitute` when construction establishes business semantics. Do not force creation, reconstruction, or state changes into generic conversion traits.
